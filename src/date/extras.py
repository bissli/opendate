...
def overlap_days(
    range_one: Interval | tuple[Date | DateTime, Date | DateTime],
    range_two: Interval | tuple[Date | DateTime, Date | DateTime],
    days: bool = False,
) -> bool | int:
    ...
    """
    if not isinstance(range_one, Interval):
        range_one = Interval(*range_one)
    if not isinstance(range_two, Interval):
        range_two = Interval(*range_two)

    latest_start = max(range_one._start, range_two._start)
    earliest_end = min(range_one._end, range_two._end)
